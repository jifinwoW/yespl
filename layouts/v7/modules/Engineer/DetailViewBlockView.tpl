						...
						<tr>
							{foreach item=FIELD_MODEL key=FIELD_NAME from=$FIELD_MODEL_LIST}
								...
								{if !$FIELD_MODEL->isViewableInDetailView()}
									{continue}
								{/if}
                                {if $FIELD_NAME eq 'rejection_reason' && $RECORD->get('eng_status') neq 'Rejected'}
                                    {continue}
                                {/if}
								...
								{else}
									...
									<td class="fieldValue {$WIDTHTYPE}" id="{$MODULE_NAME}_detailView_fieldValue_{$FIELD_MODEL->getName()}" {if $FIELD_MODEL->get('uitype') eq '19' or $fieldDataType eq 'reminder' or $fieldDataType eq 'recurrence'} colspan="3" {assign var=COUNTER value=$COUNTER+1} {/if}>
										...
										{if isset($IS_AJAX_ENABLED) && $IS_AJAX_ENABLED && $FIELD_MODEL->isEditable() eq 'true' && $FIELD_MODEL->isAjaxEditable() eq 'true' && !($MODULE_NAME eq 'Engineer' && $FIELD_NAME eq 'eng_status')}
