...
<div class="modal-dialog modelContainer">
	...
	{assign var=HEADER_TITLE value={vtranslate('LBL_ADD_STYLE', $MODULE)}}
		...
		<div class="uploadview-content container-fluid">
			<div id="create">
				<form class="form-horizontal recordEditView" name="upload" method="post" action="index.php">
					...
					<input type="hidden" name="sourceModule" value="{$SMODULE}" />
					<input type="hidden" name="sourceRecord" value="{$SID}" />
					<input type="hidden" name="relationOperation" value="true" />

					<table class="massEditTable table no-border">
						<tr>
							{assign var="FIELD_MODEL" value=$FIELD_MODELS['stylename']}
                        ...
                        {assign var="FIELD_MODEL" value=$FIELD_MODELS['stylecontent']}
                        {if $FIELD_MODELS['stylecontent']}
							</tr>
								<td class="fieldLabel col-lg-2">
									<label class="muted pull-right">
										{vtranslate($FIELD_MODEL->get('label'), $MODULE)}&nbsp;
						...
						<tr>
							{assign var=HARDCODED_FIELDS value=','|explode:"stylename,assigned_user_id,stylecontent"}
	...
	{assign var=BUTTON_ID value="js-create-style"}
