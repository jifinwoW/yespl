						...
						<tr>
							{foreach item=FIELD_MODEL key=FIELD_NAME from=$FIELD_MODEL_LIST}
								...
								{if !$FIELD_MODEL->isViewableInDetailView()}
									{continue}
								{/if}
								{assign var=ASSIGN_ROLE value=$RECORD->get('spa_assign_role')}
								{if $FIELD_MODEL->getFieldName() eq 'spa_engineer_id' && $ASSIGN_ROLE neq 'Engineer'}
									{continue}
								{/if}
								{if $FIELD_MODEL->getFieldName() eq 'spa_sc_id' && $ASSIGN_ROLE neq 'Service Cordinator'}
									{continue}
								{/if}
								...
								{else}
									...
									<td class="fieldValue {$WIDTHTYPE}" id="{$MODULE_NAME}_detailView_fieldValue_{$FIELD_MODEL->getName()}" {if $FIELD_MODEL->get('uitype') eq '19' or $fieldDataType eq 'reminder' or $fieldDataType eq 'recurrence'} colspan="3" {assign var=COUNTER value=$COUNTER+1} {/if}>
										...
										{if isset($IS_AJAX_ENABLED) && $IS_AJAX_ENABLED && $FIELD_MODEL->isEditable() eq 'true' && $FIELD_MODEL->isAjaxEditable() eq 'true' && !($MODULE_NAME eq 'SparePartsAssignment' && $FIELD_NAME eq 'spa_assign_role')}
