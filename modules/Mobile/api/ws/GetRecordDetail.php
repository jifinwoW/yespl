...
class Mobile_WS_GetRecordDetail extends Mobile_WS_FetchRecord {
	...
	protected function processWithGrouping(Mobile_API_Request $request, $response) {
		...
		$record_input = $request->get('record');
		$xIndex = strpos($record_input, 'x');
        $record_id = substr($record_input, $xIndex + 1);

		 $sql = "SELECT t.servicecoordintor, t.vendor_id, t.account_id, t.contact_id, t.engineer_id 
		 FROM vtiger_tickets t INNER JOIN vtiger_crmentity c ON c.crmid = t.ticketsid
        WHERE c.deleted = 0 AND t.ticketsid = ?";
		$result = $adb->pquery($sql, array($record_id));
		$servicecordinator_id = $adb->query_result($result,0,'servicecoordintor');
		$vendor_id = $adb->query_result($result,0,'vendor_id');
		$account_id = $adb->query_result($result,0,'account_id');
		$contact_id = $adb->query_result($result,0,'contact_id');
		$engineer_id = $adb->query_result($result,0,'engineer_id');
		...
		if ($module == 'HelpDesk') {
			...
			if(!isset($modifiedRecord['servicecoordintor'])){
				$modifiedRecord['servicecoordintor']= "41x".$servicecordinator_id;
				$sql = "SELECT firstname,lastname FROM vtiger_servicecordinator WHERE servicecordinatorid = ?";
			    $result = $adb->pquery($sql, array($servicecordinator_id));
			    $firstname = $adb->query_result($result,0,'firstname');
				$lastname = $adb->query_result($result,0,'lastname');
				$modifiedRecord['servicecoordintorLabel'] = $firstname.' '. $lastname;

			}
			if(!isset($modifiedRecord['vendor_id'])){
				$modifiedRecord['vendor_id']= "2x".$vendor_id;
				...
				$modifiedRecord['vendor_idLabel'] = $vendorname;

			}
			if(!isset($modifiedRecord['account_id'])){
				$modifiedRecord['account_id']= "11x".$account_id;
				$sql = "SELECT accountname FROM vtiger_account WHERE accountid = ?";
			    $result = $adb->pquery($sql, array($account_id));
			    $accountname = $adb->query_result($result,0,'accountname');
				$modifiedRecord['account_idLabel'] = $accountname;

			}
			if(!isset($modifiedRecord['contact_id'])){
				$modifiedRecord['contact_id']= "12x".$contact_id;
				$sql = "SELECT firstname,lastname FROM vtiger_contactdetails WHERE contactid = ?";
			    $result = $adb->pquery($sql, array($contact_id));
			    $firstname = $adb->query_result($result,0,'firstname');
				$lastname = $adb->query_result($result,0,'lastname');
				$modifiedRecord['contact_idLabel'] = $firstname.' '. $lastname;

			}
			if(!isset($modifiedRecord['engineer_id'])){
				$modifiedRecord['engineer_id']= "38x".$engineer_id;
				$sql = "SELECT engineer_name FROM vtiger_engineer WHERE engineerid = ?";
			    $result = $adb->pquery($sql, array($engineer_id));
			    $engineer_name = $adb->query_result($result,0,'engineer_name');
				$modifiedRecord['engineer_idLabel'] = $engineer_name;

			}
		
			if($modifiedRecord['location_type'] == 'Local'){
				$modifiedRecord['sla'] = 'SLA 24 HRS';
				
			}else if($modifiedRecord['location_type'] == 'Remote'){
				$modifiedRecord['sla'] = 'SLA 48 HRS';
			}else if($modifiedRecord['location_type'] == 'UpCountry'){
				$modifiedRecord['sla'] = 'SLA 72 HRS';
			}
