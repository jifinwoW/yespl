...
class Mobile_WS_GetTicketListByStatus extends Mobile_WS_Controller {
	...
	function process(Mobile_API_Request $request) {
		...
		if (!empty($headerFieldStatusValue)) {
			$additinalConditions[] = array('tickets_status', 'e', $headerFieldStatusValue);
		}

		$subCallType = $request->get('subcalltype');
		if (!empty($subCallType)) {
			$additinalConditions[] = array('subcalltype', 'e', $subCallType);
		}
		...
		$counts = $moduleModel->getTicketsByStatusCountsForUser($current_user->id, $headerFieldStatusValue, $additinalConditions);
