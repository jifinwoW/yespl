...
class Import_Data_Action extends Vtiger_Action_Controller {
		...
		for ($i = 0; $i < $numberOfRecords; ++$i) {
			...
			if ($createRecord || $mergeType == Import_Utils_Helper::$AUTO_MERGE_MERGEFIELDS || $mergeType == Import_Utils_Helper::$AUTO_MERGE_OVERWRITE) {
				...
				if (!empty($recordId)) {
					...
					// For HelpDesk, the label must be the ticket_no (auto-generated sequence number).
					// ticket_no is not available in $fieldData during import — it is generated by
					// insertIntoEntityTable() after insertIntoCrmEntity() already ran. Query the DB
					// directly to get the actual generated value.
					if ($this->module === 'HelpDesk') {
						$tnRes = $adb->pquery('SELECT ticket_no FROM vtiger_troubletickets WHERE ticketid=?', array($recordId));
						$label = $adb->query_result($tnRes, 0, 'ticket_no');
					}
	...
	public function createEntityRecord($moduleName, $entityLabel) {
		...
		// For HelpDesk, the label must be the ticket_no (auto-generated sequence number).
		// Query the DB directly since ticket_no is not present in $fieldData.
